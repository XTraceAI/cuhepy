...
The existing regression in `tests/x_vec/test_bfv_verified_client.py` documents
...
3. **Add an explicit opt-in client and benchmarks.** A future
   `src/xtrace_sdk/x_vec/crypto/bfv_attested_client.py` would keep the existing
   client style and mandatory gate. Put provider verification behind a narrow
   optional adapter; add `tests/x_vec/test_bfv_attested_client.py` and
