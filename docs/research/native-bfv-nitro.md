...
| --- | --- |
| `src/cuhepy/bfv_attested_client.py` | `BFVAttestedClient`, `BFVAttestedServer`, signed owner requests, enrollment, response gate and protected state |
| `src/cuhepy/bfv_nitro.py` | `NitroAttestationPolicy`, AWS evidence verification, official NSM C adapter |
...
| `tests/unit/test_bfv_nitro.py` | Certificate, COSE, PCR, freshness and NSM ABI regressions |
| `tests/unit/test_bfv_attested_client.py` | Owner/session bindings, receipt gate, oracle regression and lifecycle tests |
| `tests/unit/test_bfv_nitro_transport.py` | Real framed-handler round trips, malicious framing and CLI measurement compatibility |
| `tests/unit/test_bfv_nitro_aws.py` | Explicitly enabled test against a fresh real enclave |
