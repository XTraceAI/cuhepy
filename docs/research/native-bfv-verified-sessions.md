...
| --- | --- |
| `src/xtrace_sdk/x_vec/crypto/bfv_security.py` | Locally pinned parameters/limits, bounded authenticated framing, AES-GCM private exports |
| `src/xtrace_sdk/x_vec/crypto/bfv_verified_client.py` | Private result summary, client request lifecycle and public-only server wrapper |
| `tests/x_vec/test_bfv_verified_client.py` | Tampering/replay/storage/resource regressions and the residual oracle demonstration |
...
from xtrace_sdk.x_vec.crypto.bfv_security import BFVExecutionPolicy
from xtrace_sdk.x_vec.crypto.bfv_verified_client import (
    BFVVerifiedClient,
    BFVVerifiedServer,
)
...
.venv/bin/python -m pytest tests/x_vec/test_bfv_verified_client.py -q
```
