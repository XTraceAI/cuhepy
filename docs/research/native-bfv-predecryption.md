...
| File | Purpose |
| --- | --- |
| `src/xtrace_sdk/x_vec/crypto/bfv_guarded_client.py` | Owner verifier, Ed25519 receipts, mandatory pre-decryption checks and bound private exports |
| `src/xtrace_sdk/x_vec/crypto/bfv_assurance.py` | Larger-ring review profile and exact conservative circuit bounds |
| `src/xtrace_sdk/x_vec/crypto/encryption/bfv_private.py` | Python adapter to the separate private native decoder |
| `src/xtrace_sdk/x_vec/crypto/bfv_cpu_ext/private_decoder.h` | Fixed-width private NTT, CRT, rounding, locked buffers and wiping |
| `src/xtrace_sdk/x_vec/crypto/bfv_cpu_ext/private_bindings.cpp` | Bounded CPython interface, capsule ownership and GIL handling |
| `src/xtrace_sdk/x_vec/crypto/bfv_verified_client.py` | Small change: optional native private decoding; original default remains Python |
| `tests/x_vec/test_bfv_guarded_client.py` | Forgery/oracle, pin, replay, concurrency and saved-state regressions |
| `tests/x_vec/test_bfv_private.py` | Independent arithmetic, boundary, ownership and memory-lock tests |
| `tests/x_vec/test_bfv_assurance.py` | Bound checks and observed-phase comparisons |
| `tests/x_vec/native/test_bfv_private.cpp` | Standalone sanitizer, wipe and compiled secret-taint tests |
...
from xtrace_sdk.x_vec.crypto.bfv_guarded_client import (
...
.venv/bin/python -m pytest tests/x_vec/test_bfv_guarded_client.py \
  tests/x_vec/test_bfv_private.py tests/x_vec/test_bfv_assurance.py -q
```
