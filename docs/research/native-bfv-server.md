...
| --- | --- |
| `src/cuhepy/bfv/_cpu_ext/bfv_server.h` | Complete C++ public-key server and prepared masks |
| `src/cuhepy/bfv/_cpu_ext/packed_wire.h` | Direct packed ciphertext import/export |
| `src/cuhepy/bfv/_cpu_ext/profile.h` | Per-thread exclusive native timings |
| `src/cuhepy/bfv/_cpu_ext/rns_ntt.h` | Original and fast exact arithmetic modes |
| `src/cuhepy/encryption/bfv_native.py` | BFV wire interface to the native server |
| `src/cuhepy/bfv_client.py` | Backend selection through the existing client |
| `tests/unit/test_bfv_native.py` | Public-only evaluation, direct wire path, profiling and native lifetimes |
| `tests/unit/native/test_bfv_rns.cpp` | Integer arithmetic, rounding-boundary and packed-wire sanitizer oracles |
