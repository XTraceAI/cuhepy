...
Paths abbreviated to `x_vec/...` in the table are relative to `src/cuhepy/`.
...
.venv/bin/python -m pytest experiments/bfv tests/cli tests/x_vec \
  --ignore=tests/unit/test_hamming_search.py \
  --ignore=tests/unit/test_meta_search.py \
  --ignore=tests/unit/test_GPU_paillier_clients.py -q
```
