...
| `tests/x_vec/test_bfv_rns.py` | Independent arithmetic oracles, bounds, native lifetimes, concurrency and default parameters |
| `tests/x_vec/native/test_bfv_rns.cpp` | Standalone native oracles for sanitizer runs |
| `benchmarks/bfv_server.py` | Repeated native comparisons and optional SEAL comparison |

Crypto paths above are relative to `src/xtrace_sdk/x_vec/`.

## Verification and benchmarks

```bash
.venv/bin/python -m pytest tests/x_vec/test_bfv_encryption.py \
  tests/x_vec/test_bfv_evaluator.py tests/x_vec/test_bfv_rns.py \
  tests/x_vec/test_bfv_client.py -q

g++ -std=c++17 -O1 -g -fsanitize=address,undefined -fno-omit-frame-pointer \
  -I src/xtrace_sdk/x_vec/crypto/bfv_cpu_ext \
  tests/x_vec/native/test_bfv_rns.cpp -lgmpxx -lgmp -o /tmp/test_bfv_rns
/tmp/test_bfv_rns
