...
| --- | --- |
| `src/xtrace_sdk/x_vec/crypto/bfv_cpu_ext/bfv_residue.h` | Persistent RNS ciphertexts, fixed-word reconstruction and complete server |
| `src/xtrace_sdk/x_vec/crypto/bfv_cpu_ext/rns_ntt.h` | Shared NTT/key dot products and optional ciphertext-prime basis |
| `src/xtrace_sdk/x_vec/crypto/bfv_cpu_ext/bfv_server.h` | Shared layout, masks and streaming merge order |
| `src/xtrace_sdk/x_vec/crypto/bfv_cpu_ext/bindings.cpp` | Validated ABI 3 boundary and native plan selection |
| `src/xtrace_sdk/x_vec/crypto/encryption/bfv.py` | Optional product modulus and compatible key serialization |
| `src/xtrace_sdk/x_vec/crypto/encryption/bfv_rns.py` | Compiled arithmetic context and key preparation |
| `src/xtrace_sdk/x_vec/crypto/bfv_client.py` | Client configuration and backend selection |
| `tests/x_vec/test_bfv_residue.py` | Saved-key compatibility, parameters, exact search comparisons |
| `tests/x_vec/native/test_bfv_rns.cpp` | Independent integer/gadget oracles across all supported Q widths |
