...
```bash
uv run --extra bfv-nitro pytest tests/unit/test_bfv_nitro.py \
  tests/unit/test_bfv_attested_client.py tests/unit/test_bfv_nitro_transport.py
...
XTRACE_BFV_NITRO_PINS="$PWD/experiments/bfv_nitro/artifacts/measurements.json" \
  uv run --extra bfv-nitro pytest tests/unit/test_bfv_nitro_aws.py -v
```
