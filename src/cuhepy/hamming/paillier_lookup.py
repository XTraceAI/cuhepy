...
def _reject_weak_alpha(alpha_len: int, allow_weak_alpha: bool) -> None:
    """Refuse to generate a keypair whose secret exponent is brute-forceable.

    See docs/research/paillier-security.md (PL-01).
    """
    if alpha_len < MIN_ALPHA_LEN and not allow_weak_alpha:
        raise ValueError(
            f"alpha_len={alpha_len} is insecure: the secret exponent is recoverable "
            f"from the public key in ~2**{alpha_len // 2} operations. Use "
            f"alpha_len >= {MIN_ALPHA_LEN} (default {DEFAULT_ALPHA_LEN}), or pass "
            f"allow_weak_alpha=True for cryptanalysis work."
        )


def _warn_weak_alpha(alpha_len: int) -> None:
    """Warn when loading a keypair that was generated with a weak alpha."""
    if alpha_len < MIN_ALPHA_LEN:
        warnings.warn(
            f"Loaded a Paillier-Lookup key with alpha_len={alpha_len}; its secret "
            f"exponent is recoverable from the public key. Regenerate with "
            f"alpha_len >= {MIN_ALPHA_LEN}. See docs/research/paillier-security.md "
            f"(PL-01).",
            RuntimeWarning,
            stacklevel=3,
        )
...
def _load_gpu_backend() -> type[Any]:
    try:
        from cuhepy.paillier._lookup_gpu_ext import PaillierGPULookupClient
...
class PaillierLookupCPU:
    ...
    def __init__(self,embed_len:int=512, key_len:int=1024, alpha_len:int=DEFAULT_ALPHA_LEN, skip_key_gen:bool=False) -> None:
        """constructor
...
class PaillierLookupClient(HammingClientBase):
    ...
    :param alpha_len: Bit length of the secret subgroup exponent. Must be at
        least ``MIN_ALPHA_LEN`` (256) when generating a keypair -- a shorter
        exponent is recoverable from the public key by baby-step/giant-step.
    :param allow_weak_alpha: Permit ``alpha_len < MIN_ALPHA_LEN`` when generating
        keys. For cryptanalysis and regression tests only.
    """
    ...
    def __init__(
        ...
        embed_len: int = 512,
        key_len: int = 1024,
        alpha_len: int = DEFAULT_ALPHA_LEN,
        skip_key_gen: bool = False,
        device: DeviceMode = "auto",
        allow_weak_alpha: bool = False,
    ) -> None:
        if not skip_key_gen:
            _reject_weak_alpha(alpha_len, allow_weak_alpha)
        self.alpha_len = alpha_len
        self.key_len = key_len
        self.embed_len = embed_len
        self.chunk_len = key_len * 2
    ...
    def load_config(self, config: dict, precomputed_tables: dict | None = None) -> None:
        ...
        _warn_weak_alpha(self.alpha_len)
        self.chunk_len = self.key_len * 2
        if self.embed_len > self.key_len:
            self.chunk_num = 2 * self.embed_len // self.chunk_len + int((2 * self.embed_len) % self.chunk_len != 0)
        else:
