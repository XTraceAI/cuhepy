...
from cuhepy.base import HomomorphicBase
from cuhepy.types import PaillierLookupKeyPair, PaillierLookupPublicKey
...
# Security parameter for the alpha subgroup.
#
# The public key exposes g_n = g^n mod n^2, whose multiplicative order is the
# secret exponent a = lcm(q1, q2) -- an alpha_len-bit integer that decryption
# uses in place of phi. Baby-step/giant-step therefore recovers the secret key
# from the public key alone in ~2**(alpha_len/2) group operations, so alpha_len
# must be at least twice the target security level.
#
# DEFAULT_ALPHA_LEN also matches the ALPHA_LEN the CUDA extension is compiled
# with, keeping CPU- and GPU-generated keys interchangeable.
# See docs/research/paillier-security.md (PL-01).
MIN_ALPHA_LEN = 256
DEFAULT_ALPHA_LEN = 280
