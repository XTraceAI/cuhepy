...
class CudaServer final : public HammingServer {
    ...
    // A fixed upper bound keeps scratch allocation independent of index size.
    static constexpr std::size_t batch_limit = 32;
...
public:
    CudaServer(KeyHandle relin, std::map<Word, KeyHandle> keys, std::size_t padded, Word t, const mpz_class& target)
    ...
    void search(const Ciphertext& query, std::size_t count, ReadTile read_tile, Emit emit, bool compact_result) const override {
        ...
        std::size_t batch_max = std::min(batch_limit, tiles), group_max = std::min(padded,tiles);
        ...
        Buffer<U> query_rns(residues(query)), input(batch_max*6*n), lifted(batch_max*14*n), tensor(batch_max*21*n);
        ...
        for (std::size_t start = 0; start < tiles; start += padded) {
            ...
            for (std::size_t at = 0; at < group_size; at += batch_max) {
                ...
                imported.reserve(batch*6*n);
                for (std::size_t i = 0; i < batch; ++i) {
                    auto r = residues(read_tile(start+at+i)); imported.insert(imported.end(),r.begin(),r.end());
                }
                check(cudaMemcpyAsync(input.data(),imported.data(),imported.size()*sizeof(U),cudaMemcpyHostToDevice,stream.value));
                check(cudaStreamSynchronize(stream.value)); // Imported host storage may now be released.
                difference<<<blocks(batch*2*n),256,0,stream.value>>>(query_rns.data(),input.data(),lifted.data(),parameters_.data(),batch);
