...
# by the normal Python packages under `cuhepy.paillier`:
#
#   src/cuhepy/paillier/_gpu_ext/paillier_GPU_client*.so
#   src/cuhepy/paillier/_lookup_gpu_ext/paillier_GPU_lookup_client*.so
#
...
EXT_DIR="$REPO_ROOT/src/cuhepy/paillier"

if [ ! -d "$EXT_DIR/_gpu_ext" ] || \
   [ ! -d "$EXT_DIR/_lookup_gpu_ext" ]; then
    echo "Error: expected GPU source dirs under $EXT_DIR" >&2
    exit 1
fi
...
echo "  Output:    $EXT_DIR/_{,lookup_}gpu_ext/*.so"
echo
...
docker run --rm \
    ...
    /bin/bash -c '
        ...
        build_one() {
            ...
            shift
            echo "=== Building $dir ==="
            cd "/build/repo/src/cuhepy/paillier/$dir"
            make clean || true
        ...
        build_one _gpu_ext
        build_one _lookup_gpu_ext ALPHA_LEN="$ALPHA_LEN"

        chown -R "$HOST_UID:$HOST_GID" \
            /build/repo/src/cuhepy/paillier/_gpu_ext \
            /build/repo/src/cuhepy/paillier/_lookup_gpu_ext
...
echo
echo "Artifacts:"
ls -lh "$EXT_DIR/_gpu_ext/"*.so \
       "$EXT_DIR/_lookup_gpu_ext/"*.so 2>/dev/null || \
