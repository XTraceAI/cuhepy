...
from xtrace_sdk.x_vec.crypto import bfv_attested_client as attested
...
from xtrace_sdk.x_vec.crypto.encryption.bfv import BFV, _coefficient_modulus
...
def make_session(private, issuer, count=3, backend="native", enroll=True):
    ...
    if backend == "native":
        pytest.importorskip("xtrace_sdk.x_vec.crypto.bfv_cpu_ext._bfv_private")
...
def test_receipt_failures_never_parse_or_decrypt(private, trusted, monkeypatch, change):
    ...
    with monkeypatch.context() as scope:
        deny_private(scope)
        import xtrace_sdk.x_vec.crypto.bfv_security as framing
